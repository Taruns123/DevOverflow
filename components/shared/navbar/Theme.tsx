    ...
    return (
        <Menubar className="relative border-none bg-transparent shadow-none">
            <MenubarMenu>
                ...
                <MenubarContent className='min-2-[120px] dark:border-dark-400 dark:bg-dark-300 absolute -right-12 mt-3 rounded border py-2'>
                    {themes.map((item) => (
                        <MenubarItem className='dark:focus:bg-dark-400 flex items-center gap-4 px-2.5 py-2' key={item.value} onClick={() => setMode(item.value)}
                            onClickCapture={() => {
                                setMode(item.value)
